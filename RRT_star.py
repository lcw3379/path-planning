...
def check_collision(node, near_node):
    for obstacle in obstacles:
        if distance(node, Node(obstacle.position[0], obstacle.position[1])) <= obstacle.radius and distance(Node((node.x+near_node.x)/2,(node.y+near_node.y)/2),Node(obstacle.position[0], obstacle.position[1]))<= obstacle.radius:
            return True
    return False
    ...
    for node in near_list:
        if not node.parent == None:
            ...
            if node.g > new_node.g + g:
                ...
                if new_path and new_node in new_path:
                    print("modified")
...
def RRT(start, end):   
    ...
    for i in range(600000):
        ...
        if not check_collision(new_node, near_node):
        ...
        if search_goal(new_node, end_node, step_size/1.5):
            print(new_node.g)
            if min_g < new_node.g:
                continue
