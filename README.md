...
기본적인 방식은 RRT와 같지만, 트리를 재구성하는 방식이 새로 추가되었다.

1. RRT에서는 랜덤 샘플링 노드와 가장 가까운 위치의 노드를 찾았다면, RRT*는 일정 반경 안의 이웃 노드들과의 비용을 계산한 후, 가장 적은 비용의 노드를 부모 노드로 새로 정한다.
2. 이렇게 부모 노드를 새로 갱신했다면,  노드들 또한 비용을 계산해서 새로 생성한 노드와의 비용이 더 작다면 새로 생성한 노드를
